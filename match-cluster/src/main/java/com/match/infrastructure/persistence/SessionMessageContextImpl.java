...
public class SessionMessageContextImpl implements SessionMessageContext
{
        ...
        final int length)
    {
        ...
        do
        {
            ...
            {
                System.out.println("backpressure or admin action on session offer");
            }
            else if (result == Publication.NOT_CONNECTED || result == Publication.MAX_POSITION_EXCEEDED)
            {
                System.out.printf("unexpected state on session offer: %s", result);
                return;
            }
        ...
        System.out.printf("failed to offer snapshot within %s retries. Closing client session.", RETRY_COUNT);
        session.close();
    }
}
