...
public class AdminHttpApi {
    ...
    public void registerEndpoints(HttpServer server) {
        ...
        server.createContext("/api/admin/rebuild-gateway", this::handleRebuildGateway);
        server.createContext("/api/admin/rebuild-cluster", this::handleRebuildCluster);
    ...
    private void handleRebuildGateway(HttpExchange exchange) throws IOException {
        setCorsHeaders(exchange);
        if (handleCorsPreFlight(exchange)) return;
        if (!requirePost(exchange)) return;

        try {
            // Check if restart=true in query or body
            String query = exchange.getRequestURI().getQuery();
            boolean restart = query != null && query.contains("restart=true");

            adminService.rebuildGateway(restart);
            sendJsonResponse(exchange, 202, Map.of(
                "message", restart ? "Gateway rebuild + restart initiated" : "Gateway rebuild initiated",
                "restart", restart
            ));
    ...
    private void handleRebuildCluster(HttpExchange exchange) throws IOException {
        setCorsHeaders(exchange);
        if (handleCorsPreFlight(exchange)) return;
        if (!requirePost(exchange)) return;

        try {
            adminService.rebuildCluster();
            sendJsonResponse(exchange, 202, Map.of(
                "message", "Cluster rebuild initiated. Use rolling-update to deploy."
            ));
        } catch (IllegalStateException e) {
            sendJsonResponse(exchange, 409, Map.of("error", e.getMessage()));
        }
